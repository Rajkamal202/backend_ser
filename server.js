...
const ZOHO_API_BASE_URL = "https://www.zohoapis.in";
...
async function getZohoCustomerId(email, name) {
  ...
  try {
    ...
    if (searchResponse.data?.customers?.length > 0) {
      ...
      try {
        ...
        if (createResponse.data?.customer?.customer_id) {
          ...
          console.error(
            "Failed create Zoho customer, bad response:",
    ...
    if (searchError.response && searchError.response.status === 404) {
      ...
      console.log(
        "Creating Zoho customer payload (after 404):",
        JSON.stringify(createPayload)
      );
      ...
      try {
        ...
        if (createResponse.data?.customer?.customer_id) {
          ...
          console.error(
            "Failed create after 404, bad response:",
            JSON.stringify(createResponse.data)
          );
          return null;
        }
        ...
        console.error(
          "Error creating Zoho customer after 404 - Status:",
          createError.response?.status,
          "Data:",
          JSON.stringify(createError.response?.data || createError.message)
        );
...
async function createInvoiceInZoho(customerId, zohoPlanCode, amount, currency) {
    ...
    if (!zohoItemId || typeof zohoItemId !== 'string' || zohoItemId.length < 10) {
        console.error(
    ...
    try {
        const invoiceData = {
            ...
            line_items: [
                {
                    item_id: zohoItemId,
                    product_id: zohoItemId, // Keep this from last attempt
                    // --- ADD NAME AND DESCRIPTION ---
                    name: "Globodox Subscription Item", // Or fetch dynamically if possible
                    description: `Plan: ${zohoPlanCode}`, // Example description
                    // --- END ADDITIONS ---
                    quantity: 1,
                    rate: amount,
                    // You might also need tax_id here if applicable and not automatically determined by Zoho
                    // tax_id: "YOUR_ZOHO_TAX_ID_IF_NEEDED",
                }
            ]
        };
        ...
        // console.log("Using Org ID:", process.env.ZOHO_ORGANIZATION_ID); // Already in header
        ...
        // Optional: Log the payload that failed again if needed
...
async function handleTransactionCompleted(eventData) {
  try {
    ...
    const customerDetails = await getPaddleCustomerDetails(paddleCustomerId); // --- Step 2: Check if email received from Paddle ---
    ...
    if (amountFromPaddle) {
      ...
      if (!isNaN(amountInSmallestUnit)) {
        ...
        console.error(
          `Bad amount format: "${amountFromPaddle}" for TxID ${transactionId}`
        );
      }
    } else {
    ...
    if (!paddlePriceId) {
      console.error(
        `ERROR: Paddle Price ID missing in webhook data for TxID ${transactionId}. Cannot map to Zoho Plan.`
      );

      return;
    }

    console.log(`Paddle Price ID found: ${paddlePriceId}`); // --- Step 5: Map Paddle Price ID to Zoho Plan Code

    const zohoPlanCode = PADDLE_TO_ZOHO_PLAN_MAP[paddlePriceId];
    if (!zohoPlanCode || zohoPlanCode.startsWith("ZOHO_PLAN_CODE_")) {
      console.error(
        `ERROR: Could not find Zoho Plan Code mapping for Paddle Price ID ${paddlePriceId}. Check PADDLE_TO_ZOHO_PLAN_MAP.`
      );

      return;
    }

    console.log(`Mapped to Zoho Plan Code: ${zohoPlanCode}`); // --- Step 6: Call Zoho functions ---
    console.log(
      `Handling transaction: Customer=${customerEmail}, Name=${customerName}, TxID=${transactionId}, Amount=${amount} ${currency}, ZohoPlanCode=${zohoPlanCode}`
    ); // MODIFIED Log

    if (!customerEmail || amount <= 0 || !currency || !zohoPlanCode) {
      // MODIFIED Check

      console.error(
        `Missing required data before Zoho calls for TxID ${transactionId}. Stopping.`
      );

      return;
    } // Get or create Zoho customer ID

    const zohoCustomerId = await getZohoCustomerId(customerEmail, customerName); // If we got Zoho customer ID...
...
app.post("/paddle-webhook", async (req, res) => {
  ...
  try {
    ...
    if (eventType === "transaction.completed") {
      ...
      res
        .status(200)
        .send("Webhook received successfully, processing initiated.");
    } else {
      console.log(`Unhandled event type: ${eventType}`);

      res
        .status(200)
        .send(`Webhook received, unhandled event type: ${eventType}`);
    }
  } catch (error) {
...
app.listen(PORT, () => {
  ...
  if (!ZOHO_ORGANIZATION_ID)
    console.warn("Warning: ZOHO_ORGANIZATION_ID missing.");
